...
let translate (sexpr, sfuncs) =
  let ltype_of_typ = function
      A.Unit(Nat) -> i32_t
    | A.Unit(Bool) -> bool_t
    | t -> raise (Failure ("Type " ^ A.string_of_typ t ^ " not implemented.")) in
  ...
  let rec codegen_sexpr (typ, detail) = match typ with
    | A.Unit(Nat) ->
      begin
        match detail with
        | SLiteral(i) -> L.const_int nat_t i
        | SAop(sexpr1, aop, sexpr2) ->
          let lhs = codegen_sexpr sexpr1 in
          let rhs = codegen_sexpr sexpr2 in
          begin
            match aop with
